...
**What this gets you:** a support or research chatbot you can ship in an afternoon that answers "what does the law say" questions with real statute text and a source link, instead of a generic model guessing.
...
**Endpoints used:** `POST /us/statutes/search` (4 credits), `GET /us/statutes/section/{actId}/body` (6 credits).

<Tip>
  A bot answers in the time its slowest call takes. Pass `"includeBody": true`
  with `"limit": 1` on the search and the top section's full text comes back
  inline, so a reply is one round trip instead of two, at the same 4 + 6 credits.
  The two-call version below is the one to keep if you rank three candidates and
  ground on one.
</Tip>
...
<Steps>
  <Step title="Search on the user's message">
    Send the message to `POST /us/statutes/search`. Scope with `corpusType` and
...
def answer(message, state=None):
    ...
    # linked below. Copy it once; do not re-derive the field order per surface.
    source_url = best_source_url(hit)
...
<AccordionGroup>
  ...
  <Accordion title="Always show the source link" icon="link">
    Every reply ends with the citation and the official government URL. Pick that
    URL with the canonical preference order in
    [Text Formats and Source URLs](/api-guide/concepts/formats#picking-the-best-available-format)
    rather than writing your own, so the bot and the rest of your product cite
    the same page. This is what makes the bot trustworthy: users can click
    through and read the law themselves. Vaquill AI complements existing research
    tools, it does not replace a lawyer.
  </Accordion>
