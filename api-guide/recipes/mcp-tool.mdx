...
**Endpoints used:** `POST /statutes/search` (4 credits) for the custom-tool path. The hosted server path needs no direct REST calls from you.
...
<Tip>
  For heavy agent traffic, cache the free `/statutes/coverage` response and reuse it to steer `corpusType` and `state` on tool calls. That narrows searches and avoids spending credits on jurisdictions with no data.
</Tip>
