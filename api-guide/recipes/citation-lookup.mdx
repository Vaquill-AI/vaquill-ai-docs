...
<AccordionGroup>
  ...
  <Accordion title="404 on the body is refunded" icon="rotate-left">
    If `GET /statutes/section/{actId}/body` cannot return text, it responds with
    `available: false` (or a 404 that is credit-refunded). Treat that as
    unresolved-with-metadata: you have a confirmed `actId` and citation, but no
    authoritative text to quote yet.
  </Accordion>
</AccordionGroup>
