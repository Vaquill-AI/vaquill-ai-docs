...
**Endpoints used:** `POST /statutes/search` (4 credits per call). Optionally `GET /statutes/section/{actId}/body` (6 credits) when you want full text rather than the excerpt.
...
<Steps>
  ...
  <Step title="Search with rate-limit-aware pacing">
    Wrap the search call so it sleeps between requests to stay under the per-minute cap, and retries with a backoff when the API returns `429`. Failed searches (for example a `404`) are refunded, so a miss costs nothing.
  ...
  <Step title="Optionally attach full text">
    When you need the whole section rather than the excerpt, take the `actId` from a match and call the body endpoint. It charges only when text is returned.
