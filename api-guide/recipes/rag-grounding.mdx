...
**What this gets you:** an AI assistant that answers legal questions from real statute text instead of its own memory, with a citation and a link to the official source on every claim, so users can verify it instead of taking the model's word for it.
...
**Endpoints used:** `POST /us/statutes/search` (retrieve, 4 credits), `GET /us/statutes/section/{actId}/body` (full text, 6 credits per section).

<Tip>
  **You can collapse the two calls into one.** Pass `"includeBody": true` on the
  search and every hit comes back with its full text on `body`, so three grounded
  sources take one round trip instead of four. It costs the same: the 4-credit
  search plus the ordinary 6 credits for each row that actually returns text, so
  3 sourced rows is 22 credits either way, and a row whose text cannot be
  resolved comes back with `body: null` and is not charged. It buys latency, not
  a discount, and it multiplies with `limit`, so page deliberately. The two-call
  version below is written out because it is the one to use when you rank more
  candidates than you ground on.
</Tip>
...
<Steps>
  <Step title="Retrieve candidate sections">
    Send the user question to `POST /us/statutes/search`. Scope the search with
  ...
  <Step title="Fetch the full text of each section">
    ...
    `GET /us/statutes/section/{actId}/body` to get the complete `plain` text.
  </Step>
...
def answer_question(question, corpus_type=None, state=None):
    ...
    for r in results:
        ...
        # best_source_url() is the shared helper from Text Formats and Source
        # URLs, linked below. Copy it once; do not re-derive the field order.
        source_url = best_source_url(r)
        sources.append(
            {
...
<AccordionGroup>
  ...
  <Accordion title="Always link back to the official source" icon="link">
    Every result carries several optional source URL fields and any may be null.
    ...
    rather than writing your own, so the same section links to the same official
    page everywhere in your product. This is what lets your users verify the law
    themselves, and it is what keeps your product honest.
  </Accordion>
</AccordionGroup>
