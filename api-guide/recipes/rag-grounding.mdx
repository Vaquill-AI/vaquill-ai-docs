...
**Endpoints used:** `POST /us/statutes/search` (retrieve), `GET /us/statutes/section/{actId}/body` (full text).
...
<Tip>
  **You can collapse the two calls into one.** Pass `"includeBody": true` on the
  ...
  sources take one round trip instead of four. It costs the same: the search
  plus the ordinary price for each row that actually returns text, and a row
  whose text cannot be resolved comes back with `body: null` and is not
  charged. It buys latency, not
