...
These fields appear on `POST /us/statutes/search`, `GET /us/statutes/section/{actId}`, and `POST /us/statutes/sections`.
...
- **The publisher printed the credit and the scan destroyed it.** The `I&N Dec.` reporter prints a date line on every decision, but the pre-1985 volumes are OCR of paper prints and the line arrives damaged ("Docidcd by Docird Juno 25, 19.78"). We keep the publisher's printed string byte-identical in `effectiveDate` whatever state it is in, because it is the evidence, and we derive a clean date from it only where a second, independent source agrees. For that corpus the second source is the citation year on the publisher's own index, and the two disagree on 172 decisions, essentially always because the scan misread a digit. Rather than serve a year that is wrong by decades, we drop the derived date and take the year from the index. That derived date is held with the document for our own gating and is not served as its own response field.

<Note>
A decision reissued with changes is a real amendment event, and this reporter states it in the date line itself: "Decided as amended March 19, 1998".
We count it as one, so those decisions report an `amendmentsCount` of 2 rather than 1.
The reissue marker itself is held with the document and is not exposed as a response field, so read the count.
</Note>
...
### AG opinions have no amendment concept, and that is a measured finding

`corpusType=STATE_AG_OPINION` returns empty `amendmentYears` and
`amendmentsCount: 0` on every section, and that is the recorded output of a sweep
rather than an unimplemented field.

**An AG opinion is not amended.** It is issued once, on a date, and is thereafter
withdrawn, overruled, modified, or left standing. The version event this corpus
has is supersession, not amendment.

Measured across all 898 documents in the Delaware archive on 2026-09-08:

- **0.00%** carry any publisher-printed statement that the opinion itself was
  revised, reissued or withdrawn;
- **100%** carry a publisher-assigned issue date, which is what `effectiveDate`
  serves;
- 35.5% carry a CMS "modified" timestamp later than their issue date, with a
  median gap of 5 days and a maximum of 27.5 YEARS. **That is a content-management
  touch, not an amendment**, and writing it to an amendment field would
  manufacture history that never happened. It is served as `observedDate`.

So an empty `amendmentYears` here means the instrument has no amendment concept,
not that we failed to extract one.
