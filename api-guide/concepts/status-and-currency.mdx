...
<Warning>
  `goodLawStatus` is deliberately conservative. Some jurisdictions do not publish a
  reliable repeal signal, so their sections resolve to `unknown` rather than
  over-claiming `good_law`. Treat `unknown` as "verify against the official source,"
  not as "current." The field is `null` when currency checking is disabled.
</Warning>
...
<Note>
  State regulations ingested from the dated 2024 snapshot tier carry
  `actStatus: "snapshot"` and resolve to `goodLawStatus: "unknown"`. That is by
  design: those sections are not on a refresh cycle, so we do not claim they are
  current. Call [`/statutes/coverage`](/api-guide/concepts/jurisdictions) to see
  which corpora are refreshed versus snapshot.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Coverage" icon="table" href="/api-guide/coverage">
    Which corpora are refreshed versus snapshot, per jurisdiction.
  </Card>
</CardGroup>
