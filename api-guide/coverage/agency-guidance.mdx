...
description: "The 53 named federal sources behind corpusType=AGENCY_GUIDANCE, each independently filterable with source"
icon: "building-columns"
---

Sub-regulatory federal guidance: what an agency says its own rules mean, short of a
regulation. **53 named sources across 26 agencies, 51,427 documents.**

<Note>
  That total counts the **named sources on this page only**, and it is the number the table below
  sums to. The `AGENCY_GUIDANCE` corpus type holds more than this: guidance that is not attributed
  to one of the named sources carries the token without carrying a `source` you can filter on. See
  the [federal coverage table](/api-guide/coverage/federal) for the token's own total.
</Note>

All 53 sit behind the single `corpusType=AGENCY_GUIDANCE` token. Narrow to one with
...
<Note>
  ...
  authoritative over this page for **which sources exist** and what each code is called.
</Note>

## The registry

Grouped by agency. Documents are citable items (`act_id`), measured per source on 2026-09-07,
and on 2026-09-08 for the two FTC sources added that day.
Sources marked **[note]** have a caveat in [Source notes](#source-notes) that changes how you
should read their results.

<Note>
  `GET /us/statutes/coverage` computes its `AGENCY_GUIDANCE` total out of band rather than per
  request, so for up to a day after a source is ingested it can report fewer documents than this
  page.
  The column above sums to 86,900, which is exactly what `GET /us/statutes/coverage` reports for
  `AGENCY_GUIDANCE`. Every row is a served count, not an ingested one. The distinction matters most
  on IRS written determinations, where 45,358 documents are ingested and 35,485 are reachable
  through the API, the difference being 9,873 OCR-read scans. The endpoint stays authoritative.
</Note>

| Source | `source` | Documents | Coverage |
|---|---|---:|---|
| SSA Rulings | `ssa_ruling` | 1,116 | - |
...
| IRS Written Determinations **[note]** | `irs_written_determination` | 35,485 served | 1999-2026 |
...
| FTC HSR Premerger Interpretations **[note]** | `ftc_hsr_interpretation` | 4,589 | 1978-2025 |
| FTC Advocacy Filings **[note]** | `ftc_advocacy_filing` | 860 | 1985-2026 |
| Copyright Office Circulars | `copyright_circular` | 58 | - |
| Copyright Office Compendium (Third) **[note]** | `copyright_compendium` | 5,920 | 3 editions, 2014-2021 |
| CMS Medicare Internet-Only Manuals **[note]** | `cms_iom` | 11,018 | 2000-2026 |
...
| NLRB General Counsel Memoranda | `nlrb_gc_memo` | 301 | 1973-present |
| NLRB Division of Advice Memoranda | `nlrb_advice_memo` | 340 | Rolling 10-year window |
| CFTC Staff Letters | `cftc_staff_letter` | 944 | 2008-present |
| Federal Reserve SR and CA Letters | `frb_sr_letter` | 342 | Since 1990 |
| OCC Bulletins | `occ_bulletin` | 612 | Since 1994 |
| OCC Interpretive Letters | `occ_interpretive_letter` | 448 | 1996-present |
| FDIC Financial Institution Letters | `fdic_fil` | 2,321 | Since 1994 |
...
| OFAC Sanctions FAQs **[note]** | `ofac_faq` | 1,362 | Revisions since 2023-03-22 |
...
| FERC Policy Statements **[note]** | `ferc_policy_statement` | 82 | Since 1996 |
| DOE Appliance Standards Guidance | `doe_appliance_guidance` | 60 | Draft guidance excluded |
| HHS OCR HIPAA Guidance | `hhs_ocr_hipaa_guidance` | 44 | - |
| HHS OCR HIPAA FAQs | `hhs_ocr_hipaa_faq` | 444 | Deduplicated across topic pages |
| HHS OCR Resolution Agreements and CMPs | `hhs_ocr_resolution_agreement` | 145 | - |
...
<Accordion title="irs_written_determination - by statute, not citable as precedent">
  ...
  **Each is directed only to the taxpayer who requested it, and § 6110(k)(3) says it "may not be
  ...
  Coverage runs **1999-01-08 to 2026-09-04**, 28 distinct years. **35,485 determinations are
  served, 78.1% of the 45,431 the IRS lists.** The 9,873 OCR-read scans are ingested but are not
  ...
  **9,873 of these were read by OCR rather than from a text layer**, because the IRS publishes them
  as scans of signed paper. Those are recorded ingest-side as `extractionMethod: "ocr"`, which the API does not currently return; the rest
...
<Accordion title="eeoc_guidance - the retirement banner is the only signal EEOC publishes">
  ...
  **The enumeration is EEOC's own sitemap** filtered to `/laws/guidance/`, which yields all 179
  ...
  a strict subset, so building from the hub would ship a third of the corpus and report success.
  ...
  **`act_id` is the URL slug, deliberately not EEOC's OLC control number**: 178 documents print a
  ...
  `corpusType=AGENCY_ADJUDICATION`, and they are reachable only through a search path the agency
  restricts automated access to.
</Accordion>
...
<Accordion title="ftc_hsr_interpretation - one series, two halves, and 1997 is missing at the source">
  The Hart-Scott-Rodino premerger notification interpretations, both halves under one
  `source`. A **formal** interpretation is the Commission's own authoritative construction of a
  Premerger Notification Rule under 16 C.F.R. 803.30: a closed series of 20, issued 1978 to
  2000, with none since. An **informal** interpretation is the Premerger Notification Office
  answering a practitioner's question about the same rules: 4,569 of them, 1983 to 2025.
  Filter the two apart with `documentSubtype`; join them on `construedRule`, which carries the
  rule number the interpretation is about on 99.0% of documents.

  🔴 **1997 is entirely absent, at the publisher.** The FTC serves no informal interpretation
  numbered for that year at all, against 141 in 1996 and 83 in 1998, so roughly 100 to 140
  interpretations from 1997 are missing from the source rather than from this corpus.

  The FTC's own listing counts 4,759, which is a LINK count: 186 interpretations are served
  twice under a duplicate page. Each is held once, under the interpretation number the
  Commission assigns.

  An informal interpretation is **staff views, not Commission action**, and is expressly not
  binding on the Commission. Where the record also attaches an "Original Image" scan of the
  same letter, it is listed in `relatedDocuments` and not mirrored: it is a redacted photocopy
  of text the document already carries in full.

  `goodLawStatus` is **UNKNOWN** for this source. The Commission states a supersession in the
  slug of one interpretation and publishes three dated versions of another; those relations are
  served as `supersedes` / `supersededBy`, and no document is marked dead on the strength of
  them. `effectiveDate` is absent on 13 of the 20 formal interpretations, because the FTC
  publishes no date field for that series and those 13 print dates only inside their own
  argument, where the document's own date cannot be told apart from the dates it cites.
</Accordion>

<Accordion title="ftc_advocacy_filing - advisory only: these bind nobody">
  Comments and letters the Commission and its staff address to **state legislatures, state
  supreme courts, governors and other federal agencies** about the competition effects of a
  proposed measure. 860 filings, 1985 to 2026.

  🔴 **They bind nobody.** Unlike every other source under `AGENCY_GUIDANCE`, an advocacy
  filing is not the agency telling the regulated public how it reads a law it administers: it
  decides no case, construes no FTC rule, and the recipient is free to ignore it. It is held as
  its own `source` with its own `forum` (`ftc_advocacy`) precisely so it can be scoped out in
  one parameter, and every document says so in its own `coverageNote`.

  The Matter Number is a **program-level** number the FTC reuses across many filings (one
  covers 29 different documents), so it identifies a program, not a document, and is served as
  `caseNumber` rather than as a citation.

  `goodLawStatus` is **UNKNOWN** for this source.
</Accordion>
...
<Accordion title="ofac_faq - prior versions are separate, citable documents">
  ...
  **The revision trail starts 2023-03-22 and goes no further back.** OFAC migrated its FAQs to a
  ...
  original publication date, so 860 version records carry an ingest-side `observedDate`, not returned by the API, with no
  `effectiveDate`.
