...
<Tip>
  To pull up one specific section, pass its citation as the query (for example `42 U.S.C. § 1983`) and leave `matchType` at `any`. It resolves to that section at rank 1. `GET /us/statutes/resolve` does the same job for less when you already hold a well-formed citation.
</Tip>
...
- `includeBody`: attaches each hit's full text on `body` in the same call. It bills each row that returns text on top of the search, so it multiplies with `limit`. Use it with a small `limit`, and read `creditsConsumed`.
...
- On `429`, respect the `Retry-After` header and use exponential backoff. See [Errors](/api-guide/errors).
