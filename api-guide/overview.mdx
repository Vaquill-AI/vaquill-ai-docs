...
Every path is prefixed with `/api/v1`. Search and retrieval endpoints cost credits (1 credit = \$0.01). Discovery endpoints are free.

| Method | Path | Purpose | Credits |
|--------|------|---------|---------|
| `POST` | `/statutes/search` | Hybrid keyword + semantic search across the corpus | 4 |
| `GET` | `/statutes/section/{actId}` | Section metadata by `actId` | 2 |
| `GET` | `/statutes/section/{actId}/body` | Full section text (HTML + plain) | 6 |
| `GET` | `/statutes/states` | Coverage summary by jurisdiction | Free |
| `GET` | `/statutes/codes?state=XX` | Statutory codes ingested for one state | Free |
| `GET` | `/statutes/coverage` | Full coverage matrix + corpus legend | Free |
| `GET` | `/api-credits/pricing` | Credit cost per endpoint (no auth) | Free |
...
<CardGroup cols={2}>
  ...
  <Card title="Credits & pricing" icon="coins" href="/api-guide/credits">
    What each call costs and how credits work.
  </Card>
</CardGroup>
