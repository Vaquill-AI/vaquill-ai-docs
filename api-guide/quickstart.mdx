...
<Steps>
  <Step title="Get an API key">
    Create a key in your [dashboard](https://app.vaquill.ai/settings) under **API Keys**. Copy it once, it will not be shown again. See [Authentication](/api-guide/authentication) for details. New accounts get a 500 credit signup bonus (\$5).
  </Step>
  ...
  <Step title="Fetch the section and its body">
    ...
    <Tip>
      The body endpoint returns `html` and `plain` text plus an `available` flag and a `source`. You are charged only when text is returned. A 404 or unavailable section is refunded.
    </Tip>
  </Step>

  <Step title="Discover coverage">
    The corpus grows weekly. Call the free `GET /statutes/coverage` to see the corpus legend and, for each jurisdiction, which `corpusType` tokens have data. Pass one of those tokens back into `/statutes/search`.
...
<CardGroup cols={2}>
  ...
  <Card title="Credits & pricing" icon="coins" href="/api-guide/credits">
    Understand what each call costs and how refunds work.
  </Card>
