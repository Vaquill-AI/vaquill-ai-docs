...
Every path below is prefixed with `/api/v1` and every request needs an `Authorization: Bearer vq_key_...` header. Every endpoint in this guide is **free** except one: [`GET /watches/{id}/changes/{changeId}/diff`](#read-the-diff-for-one-change) costs **4 credits** per call, because it reconstructs both sides of the text. Everything else is authenticated and rate-limited but not credit-metered.

<Warning>
  **Two plan limits apply to watches.**

  **Webhook delivery requires the Business plan.** Creating or repointing a watch whose `channel` is `webhook` or `both` returns **403** on any other plan. Email delivery is open to every plan, so `channel: "email"` always works.

  **Watch count is capped at 3** outside Business, and at 100 on Business. Exceeding it returns **429**. Delete a watch before creating another, or upgrade.
</Warning>
...
def all_boards():
    """Page /boards. The default limit is also its maximum, so do not skip this."""
    boards, offset = [], 0
    while True:
        data = requests.get(
...
<Warning>
  **`GET /boards` truncates silently once the registry passes 500.** `limit` defaults to 500 and 500 is also the maximum, so a call that never passes `offset` returns the first 500 boards and no error. There are about 216 boards today, so the default covers the whole registry right now, but that is a fact about today. Read `data.total` and `data.hasMore` and page on `offset`, as above.

  `GET /boards` is free, so paging it costs nothing but a request against your [rate limit](/api-guide/rate-limits).
</Warning>
...
| Parameter | Notes |
| --- | --- |
| `sinceId` | Return changes with an `id` **greater** than this. The cursor to build on, for walking forward. |
| `beforeId` | Return changes with an `id` **less** than this. How you page backwards into history. |
| `since` | ISO-8601 timestamp, strictly after. Combines with `sinceId`. See below. |
...
`since` is coarser than `sinceId`, because one refresh writes many rows at the same instant, but it is what you want if all you kept was a `detectedAt` from a webhook body.
...
`sinceId` only walks forward, so it cannot get you the history that already existed when you created the watch. `beforeId` is how you backfill.

Keep `order=desc` (the default), and pass the **lowest** `id` you have seen as `beforeId` to fetch the page behind it. Repeat until a page comes back empty.
...
while True:
    ...
    changes = page["data"]["changes"]
    if not changes:
        break
    history.extend(changes)

    # meta.cursor is the HIGHEST id on the page, so it is the wrong end for
    # walking backwards. Take the lowest id yourself.
...
```json
{
  ...
  "meta": { "processingTimeMs": 41.0, "creditsConsumed": 4 }
}
```
