...
- **Automated lookup pipelines** - connect a form to Vaquill AI for instant statute citations
...
- **Client-intake triage** - route incoming statute questions through Vaquill AI, then forward results to your team

## Prerequisites

1. **Vaquill AI account** - sign up at [app.vaquill.ai](https://app.vaquill.ai)
...
| query | results | citations | jurisdiction | status |
|---|---|---|---|---|
| civil action for deprivation of rights | *(filled by Vaquill AI)* | | | |
| motion to dismiss for failure to state a claim | *(filled by Vaquill AI)* | | | |
| overtime exemption for executive employees | *(filled by Vaquill AI)* | | | |

## Vaquill API reference

```
POST https://api.vaquill.ai/api/v1/us/statutes/search
...
Change the request body `jurisdiction` field to target a specific code: `"us"` covers federal sources (the US Code and the CFR), and any state code (e.g. `"ca"`, `"ny"`, `"tx"`) restricts the search to that state's statutes. The corpus spans all 50 states plus DC and Puerto Rico, the US Code (60,170 sections across 53 titles), and the CFR (219,114 sections across 49 titles).
...
Make.com is cloud-only. The free tier covers ~1,000 operations per month, which is enough for small batches. Paid plans scale up.

Make.com counts each HTTP module call and each sheet row read / written, so a single Vaquill AI search typically consumes 3 operations (read row, call API, write row).
...
<CardGroup cols={2}>
  <Card title="Statutes Search API" icon="magnifying-glass" href="/api-reference/us-statutes/search-us-statutes-constitutions-court-rules-and-executive-actions">
