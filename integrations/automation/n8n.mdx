...
Make.com counts each HTTP module call and each sheet row read / written, so a single Vaquill search typically consumes 3 operations (read row, call API, write row).
