...
The `AgentCapability` enum (`fastest-responses`, `optimal-choice`, `advanced-reasoning`, `complex-tasks`) maps onto Vaquill AI's `mode`: the first two go to `standard`, the latter two go to `deep`.
...
<CardGroup cols={2}>
  ...
  <Card title="Statutes API" icon="message" href="/api-reference/us-statutes/search-us-statutes-constitutions-court-rules-and-executive-actions">
