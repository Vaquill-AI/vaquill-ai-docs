...
description: "MCP servers, chatbots, widgets, browser extensions, and automation templates that connect Vaquill AI to the tools your team already uses."
icon: "plug"
...
Put Vaquill AI wherever your team already works - Claude, Slack, a marketing site, a Google Sheet. Every integration is open-source, self-hostable, and calls the same [Vaquill statutes API](/api-reference/us-statutes/search-us-statutes-constitutions-court-rules-and-executive-actions) under the hood.

<CardGroup cols={2}>
  <Card title="MCP Servers" icon="plug" href="/integrations/mcp">
    Plug Vaquill AI's full US statute corpus (US Code, CFR, all 50 state codes) into Claude, Cursor, VS Code, or any MCP client.
  </Card>
...
flowchart LR
    ...
    B -->|"POST /api/v1/us/statutes/search"| C["**Vaquill statutes API**"]
...
2. `POST /api/v1/us/statutes/search` with `{ "query": "..." }` and `Authorization: Bearer vq_key_...`.
