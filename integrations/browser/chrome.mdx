...
A Manifest V3 Chrome extension that puts Vaquill AI's statute search in any browser tab. The popup calls `api.vaquill.ai` directly using the user's own API key - no proxy server, no backend to run, no infrastructure to maintain.
...
Full schema in the [Statute search API reference](/api-reference/us-statutes/search-us-statutes-constitutions-court-rules-and-executive-actions).
...
extension/
  ...
  js/
    ...
    background.js        Service worker - calls POST /api/v1/us/statutes/search
...
<CardGroup cols={2}>
  <Card title="Statute search API" icon="message" href="/api-reference/us-statutes/search-us-statutes-constitutions-court-rules-and-executive-actions">
