...
An integration that creates clients, matters, folders, documents, drafts, templates, chats, comparisons, matrices, reviews and workflow runs needs to be able to undo that. This page is how.
...
Some resources go to a trash a person can see in the app. Drafts and comparisons do; documents, clients, matters, folders, reviews, workflow runs, matrices, chats and templates do not. A chronology event is soft-deleted in the database and is not restorable from anywhere you can reach.
...
Every `DELETE` this API publishes. Fifteen of them, derived from the route registry on 2026-09-06.
...
| Matrix row | `DELETE /v1/matters/{matterId}/matrices/{matrixId}/rows/{rowId}` | Removes one document from the grid with every answer for it. The document is untouched |
| Matrix column | `DELETE /v1/matters/{matterId}/matrices/{matrixId}/columns/{columnId}` | Removes one question with every answer to it. Refused with `409` while another column depends on it |
...
| Chat | `DELETE /v1/matters/{matterId}/chats/{chatId}` | Takes its messages, threads, reading outlines, edit history and verifications with it |
| Chronology event | `DELETE /v1/matters/{matterId}/chronology/events/{eventId}` | A soft delete: it stops being served everywhere and the row is not destroyed |
| Template | `DELETE /v1/templates/{templateId}` | Not matter-scoped. Runs that already used it keep working and stay readable |
...
Matrix row and column positions are not renumbered after a delete, so `position` stays ascending and stops being contiguous.

Deleting a chat is refused with `409` while an ask, a deep-research run or a verification on it is still running.

Deleting a chronology event is not the only way one disappears: deleting the **document** an event was extracted from hard-deletes every event derived from it, past this soft delete and with no record.

**Playbooks cannot be deleted through this API.**
There is no `playbooks.delete` operation, and a `DELETE` against a playbook path answers `405 method-not-allowed`.
A playbook you no longer want can be emptied with `playbooks.update`, or deleted by its owner in the app.
The gap is deliberate for now: a review records the playbook it ran against as a plain JSONB key with no foreign key, so nothing in the database stops a deletion from leaving twelve completed reviews pointing at a row that is gone.
...
Most deletes carry the resource's write scope. Five do not, because the scope taxonomy has no write scope for them:

| Operation | Scope |
| --- | --- |
| `drafts.delete` | `drafting:run` |
| `templates.delete` | `drafting:run` |
| `comparisons.delete` | `compare:run` |
| `reviews.delete` | `review:run` |
| `chats.delete` | `research:run` |

`:run` reads as "starting work" and these are deletes. The taxonomy is a frozen contract, and adding `compare:write` would make the operation answer `403` for every credential already issued, including yours. If you are minting a credential for a cleanup job, grant the `:run` scope for those five.

Two more are worth checking before you mint that credential. `folders.delete` carries `matters:write`, because folders have no scope of their own, and `workflowRuns.delete` carries `workflows:write`, which is not implied by the `workflows:run` a launching integration already holds.
...
<Steps>
  <Step title="Delete the runs and results">
    `comparisons.list`, `reviews.list`, `workflowRuns.list` (`GET /v1/matters/{matterId}/workflow-runs`) and `matrices.list` inside the matter, then delete each. Anything still running answers `409 operation-in-flight`; poll it and come back.
  </Step>
  <Step title="Delete the chats">
    `chats.list` inside the matter, then delete each. A chat is a blocker on the matter and is easy to forget, because your integration may never have created one deliberately.
  </Step>
