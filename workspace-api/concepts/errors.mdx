...
**The `type` is a real URL.**
Open it and it lands on a page for that exact error: what it means, and what to do about it.
Every type has one.
...
| 503 | `credential-store-unavailable`, `dispatch-unavailable`, `run-guard-unavailable`, `storage-unavailable` | Ours, not yours. Retry with backoff |
...
The types above are the ones every integration meets.
The rest are the specific not-found and conflict types for one resource, and they read exactly as you would expect: `draft-not-found`, `matrix-not-found`, `upload-not-completable`, `review-not-finished`.
The capability pages name the ones that particular capability can raise, and the `type` URL on any problem document lands on that slug's own page, so there is nothing to look up in a list.

The general rule holds everywhere: a `4xx` slug describes something you can fix, and a `5xx` slug describes something we have to fix.
There are six `5xx` slugs: `internal-error` (500), `web-research-unavailable` (502), and the four `503`s in the table above.
Nothing in either group is worth parsing out of the `detail` string.
