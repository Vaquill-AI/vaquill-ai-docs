...
<Note>
`category` is a slug rather than an open string, but it is **not published as an enum** in the schema. The underlying vocabulary still carries categories from an earlier, non-US product that this API does not sell, and publishing the enum would publish those as accepted values. Common slugs are `nda`, `employment`, `service`, `dpa`, `baa`, `statement_of_work`, `software_license` and `settlement_agreement`. An unknown slug comes back as `422 invalid-request` naming `body.category`, **before** anything is queued, so probing costs you nothing but a round trip.
</Note>
...
| `source` | How the draft came to exist: `generated`, `uploaded` or `imported` |
...
<Note>
Templates that belong to **one person** rather than to the organization are not listed. That is not an omission: an organization's machine credential has no business reading an employee's private document. If a template you expect is missing, it was saved personally and needs to be shared to the organization in the web app.
</Note>

The template's own body and its variable definitions are not published. What a headless caller needs is which template to name and what came back after it ran.
