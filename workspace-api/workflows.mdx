...
Six calls cover the whole surface. List the catalog, launch a run, poll the operation, fetch the artifacts, list the runs in a matter, delete one.
...
## Listing the runs in a matter

```bash
curl "https://api.vaquill.ai/workspace/v1/matters/mat_.../workflow-runs?limit=50" \
  -H "Authorization: Bearer vq_ws_..."
```

Every run in that matter, newest first, in the standard page envelope. This is how you recover a run id you lost, and how a cleanup job enumerates what it has to remove.

Runs started in the app outside any matter are not here and cannot be: every route on this resource is matter-nested, so a flat list would name rows this API has no URL for. Everything started through this API appears.

## Deleting a run

```bash
curl -X DELETE "https://api.vaquill.ai/workspace/v1/matters/mat_.../workflow-runs/wfr_..." \
  -H "Authorization: Bearer vq_ws_..."
```

Answers `204`, and removes the run and its event history.

**Documents the run produced are not deleted.** They stay filed in the matter and simply stop pointing back at the run, so deleting the record of the work does not delete the work.

Refused with `409 operation-in-flight` while the run has not finished; poll the operation that started it first. There is no undo, and a repeated delete answers `404`, so on a delete that timed out treat a subsequent `404` as success.

## Every operation on this page

Six, derived from the route registry on 2026-09-06. Nothing about workflows lives outside this table.

| Call | Does | Scope |
| --- | --- | --- |
| `GET /v1/workflows` | Lists the fixed catalog of definitions | `workflows:read` |
| `POST /v1/matters/{matterId}/workflows/{workflowId}/runs` | Launches a run, answers `202` | `workflows:run` |
| `GET /v1/matters/{matterId}/workflow-runs` | Lists the runs in a matter, newest first | `workflows:read` |
| `GET /v1/matters/{matterId}/workflow-runs/{runId}` | Reads one run and its artifact list | `workflows:read` |
| `GET /v1/matters/{matterId}/workflow-runs/{runId}/artifact` | Signs a 15-minute URL for one deliverable | `exports:read` |
| `DELETE /v1/matters/{matterId}/workflow-runs/{runId}` | Deletes the run record | `workflows:write` |

<Warning>
Deleting a run costs **`workflows:write`**, which `workflows:run` does not imply. A credential that can launch runs cannot necessarily remove them. `workflows:write` does imply `workflows:read`.
</Warning>
...
`workflows:run` does not imply `workflows:read`, so a credential that only starts runs cannot enumerate the catalog.
Most integrations want both, plus `exports:read` if they collect the output and `workflows:write` if they clean up afterwards.
